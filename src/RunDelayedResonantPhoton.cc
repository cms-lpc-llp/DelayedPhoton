...
void usage()
{
  std::cerr << "Usage: RunDelayedResonantPhoton  <input list>  [options]\n[options]:\n"
	    << "-d  --isData\n"
...
int main(int argc, char* argv[]){
  ...
  bool isData = false;
  if ( _isData == "yes" || _d == "yes" || _isData == "y" || _d == "y" ) isData = true;
  ...
  std::cout << "[INFO]: optionalLabel --> " << label << std::endl;
    ...
    DelayedResonantPhoton analyzer(theChain);
    
    //------ EXECUTE ------//
    cout << "[INFO] Executing DelayedResonantPhoton..." << endl;
