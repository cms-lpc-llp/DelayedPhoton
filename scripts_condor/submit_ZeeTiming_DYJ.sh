...
do
	echo "Sample " ${sample}
	inputfilelist=/src/DelayedPhoton/lists/DelayedPhoton2016/MC2016/${sample}.txt
	...
	do
		...
		echo "Arguments = ${analyzer}_${sample}_Job${jobnumber}_Of_${maxjob} /store/group/phys_llp/ZeeTiming/2016/jobs ${analyzer} ${inputfilelist} no 10 ${filesPerJob} ${jobnumber} ${sample}_Job${jobnumber}_Of_${maxjob}.root" >> ${jdl_file}
		...
		echo "x509userproxy = \$ENV(X509_USER_PROXY)" >> ${jdl_file}
                ...
                echo "+JobBatchName = \"DrellYan2016_ZeeTiming\"" >> ${jdl_file}
