...
do
	...
	do
		...
		echo "Arguments = ${analyzer}_${sample}_Job${jobnumber}_Of_${maxjob} /store/group/phys_susy/razor/Run2Analysis/DelayedPhotonAnalysis/2018D/jobs/ ${analyzer} ${inputfilelist} no 10 ${filesPerJob} ${jobnumber} ${sample}_Job${jobnumber}_Of_${maxjob}.root" >> ${jdl_file}
		...
		echo "x509userproxy = \$ENV(X509_USER_PROXY)" >> ${jdl_file}
                ...
                echo "+Requirements=(TARGET.OpSysAndVer==\"CentOS7\" && regexp(\"data-.*\", TARGET.Machine))" >> ${jdl_file}
